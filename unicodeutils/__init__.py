...
def encode(value, encoding=None): #{{{1
...
def decode(value, encoding=None): #{{{1
    ...
    if is_list(value):
        return list(decode(v) for v in value)
...
def strip_non_word(value, replace=None, allowed=None): #{{{1
    ...
    return u''.join(map(
        lambda s: (s in pool and s) or (replace or u' '), value))
...
def strip_non_ascii(value, replace=None): #{{{1
    ...
    """
    return u''.join(map(
        lambda s: (s <= u'~' and s) or (replace or u' '), value))
...
def strip_non_windows(value, replace=None): #{{{1
    ...
    """
    return u''.join(map(
        lambda s: (s not in u':*?;"<>|' and s) or (replace or u' '), value))
...
def normalize(value): #{{{1
    ...
    u'aou'
    """
    return unicode(unicodedata.normalize('NFKD', value
        ).encode('ascii', 'ignore'), encoding='utf-8')
...
def sanitize(value): #{{{1
    ...
    u'foo_bar_baz'
    """
    return squeeze(normalize_symbols(strip_non_word(normalize(
        value.replace('.', ' ')))).lower().replace(' ', '_'), '_')
