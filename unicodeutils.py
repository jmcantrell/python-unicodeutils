...
def encode(value, encoding=ENCODING):  # {{{1
    ...
    if is_list(value):
        return list(encode(v, encoding) for v in value)
    # If dict, recursively encode keys and values
    if is_dict(value):
        return dict((encode(k, encoding), encode(v, encoding)) for k, v in value.items())
...
def decode(value, encoding=ENCODING):  # {{{1
    ...
    if is_list(value):
        return list(decode(v, encoding) for v in value)
    # If dict, recursively decode keys and values
    if is_dict(value):
        return dict((decode(k, encoding), decode(v, encoding)) for k, v in value.items())
